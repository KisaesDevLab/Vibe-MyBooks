...
const TransactionRangeReport = lazyNamed(() => import('./features/reports/TransactionRangeReport'), 'TransactionRangeReport');
      ...
      <BrowserRouter basename={import.meta.env.BASE_URL}>
        ...
        <Routes>
          ...
          >
            ...
            <Route path="/reports/sales-by-customer" element={<GenericReport title="Sales by Customer" endpoint="sales-by-customer" useTagFilter useBasis columns={[{key:'customer_name',label:'Customer'},{key:'total',label:'Total',align:'right',format:'money',drillDown:drillByContact('contact_id')}]} />} />
            <Route path="/reports/sales-by-item" element={<GenericReport title="Sales by Item" endpoint="sales-by-item" useTagFilter useBasis columns={[{key:'item_sku',label:'SKU'},{key:'item_name',label:'Item'},{key:'txn_count',label:'Txns',align:'right'},{key:'total',label:'Total',align:'right',format:'money'}]} />} />
            {/* Dev-only visual gallery for <SplitRowV2> states (build-plan Phase 6). */}
            {import.meta.env.DEV && (
              <Route path="/__dev/split-row-v2" element={<SplitRowV2GalleryPage />} />
            )}
            <Route path="/reports/ap-aging-summary" element={<GenericReport title="AP Aging Summary" endpoint="ap-aging-summary" useDateRange={false} useAsOfDate useTagFilter columns={[{key:'vendor_name',label:'Vendor'},{key:'current',label:'Current',align:'right',format:'money'},{key:'bucket1to30',label:'1-30',align:'right',format:'money'},{key:'bucket31to60',label:'31-60',align:'right',format:'money'},{key:'bucket61to90',label:'61-90',align:'right',format:'money'},{key:'bucketOver90',label:'90+',align:'right',format:'money'},{key:'total',label:'Total',align:'right',format:'money',drillDown:drillByContact('contact_id')}]} dataKey="vendors" />} />
            <Route path="/reports/ap-aging-detail" element={<GenericReport title="AP Aging Detail" endpoint="ap-aging-detail" useDateRange={false} useAsOfDate useTagFilter columns={[{key:'txn_number',label:'Bill #',drillDown:drillToTxn('id')},{key:'vendor_name',label:'Vendor'},{key:'vendor_invoice_number',label:'Vendor Inv #'},{key:'txn_date',label:'Date'},{key:'due_date',label:'Due'},{key:'days_overdue',label:'Days Overdue',align:'right'},{key:'balance',label:'Balance',align:'right',format:'money'}]} dataKey="details" />} />
            ...
            <Route path="/reports/transaction-report" element={<TransactionRangeReport />} />
            <Route path="/reports/trial-balance" element={<GenericReport title="Trial Balance" endpoint="trial-balance" useDateRange={true} useTagFilter useBasis columns={[{key:'account_number',label:'#'},{key:'name',label:'Account'},{key:'account_type',label:'Type'},{key:'debit',label:'Debit',align:'right',format:'money',drillDown:drillByAccount('id')},{key:'credit',label:'Credit',align:'right',format:'money',drillDown:drillByAccount('id')}]} totalsFrom={{debit:'totalDebits',credit:'totalCredits'}} />} />
            <Route path="/reports/account-activity-summary" element={<GenericReport title="Account Activity Summary" endpoint="account-activity-summary" useDateRange={true} useTagFilter columns={[{key:'account_number',label:'#'},{key:'name',label:'Account'},{key:'account_type',label:'Type'},{key:'total_debit',label:'Total Debits',align:'right',format:'money',drillDown:drillByAccount('id')},{key:'total_credit',label:'Total Credits',align:'right',format:'money',drillDown:drillByAccount('id')},{key:'net',label:'Net',align:'right',format:'money'}]} totalsFrom={{total_debit:'totalDebits',total_credit:'totalCredits',net:'totalNet'}} />} />
            <Route path="/reports/transaction-list" element={<GenericReport title="Transaction List" endpoint="transaction-list" useTagFilter useBasis columns={[{key:'txn_date',label:'Date'},{key:'txn_type',label:'Type'},{key:'txn_number',label:'Number',drillDown:drillToTxn('id')},{key:'contact_name',label:'Contact'},{key:'account',label:'Account'},{key:'amount',label:'Amount',align:'right',format:'money'},{key:'memo',label:'Memo'}]} />} />
            <Route path="/reports/journal-entry-report" element={<GenericReport title="Journal Entries" endpoint="journal-entry-report" useTagFilter columns={[{key:'txn_date',label:'Date'},{key:'txn_number',label:'Number',drillDown:drillToTxn('id')},{key:'account',label:'Account'},{key:'debit',label:'Debit',align:'right',format:'money'},{key:'credit',label:'Credit',align:'right',format:'money'},{key:'memo',label:'Memo'}]} />} />
            ...
            <Route path="/admin/tunnel" element={<AdminRoute><TunnelConfigPage /></AdminRoute>} />
