...
export function PlaidConnectionsMonitorPage() {
  const { data: stats } = useQuery({
    ...
    queryFn: () => apiClient<any>('/admin/plaid/stats'),
  });

  const { data: connData, isLoading } = useQuery({
    queryKey: ['admin', 'plaid-connections'],
    queryFn: () => apiClient<any>('/admin/plaid/connections'),
  });

  const { data: logData } = useQuery({
    queryKey: ['admin', 'plaid-webhook-log'],
    queryFn: () => apiClient<any>('/admin/plaid/webhook-log'),
  });
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {(connData?.connections || []).map((conn: any) => (
                  <tr key={conn.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-600">{conn.accounts?.length || 0} ({conn.accounts?.filter((a: any) => a.isMapped).length || 0} mapped)</td>
            ...
            <tbody className="divide-y divide-gray-100">
              {(logData?.logs || []).map((log: any) => (
                <tr key={log.id} className="hover:bg-gray-50">
                  <td className="px-4 py-2 text-gray-600 text-xs">{new Date(log.receivedAt).toLocaleString()}</td>
