...
function InlineFormat({ text }: { text: string }) {
  ...
  while (remaining.length > 0) {
    ...
    if (earliest.type === 'bold') {
      parts.push(<strong key={key++} className="font-semibold text-gray-900">{earliest.inner}</strong>);
    } else {
      parts.push(<code key={key++} className="bg-gray-100 px-1 py-0.5 rounded text-xs font-mono text-gray-800">{earliest.inner}</code>);
    }
      ...
      <table className="w-full text-sm border border-gray-200 rounded-lg">
        <thead>
          <tr className="bg-gray-50 border-b border-gray-200">
            {headers.map((h, i) => (
              <th key={i} className="text-left px-3 py-2 font-medium text-gray-600 text-xs">
                <InlineFormat text={h} />
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {rows.map((row, ri) => (
            <tr key={ri} className="border-b border-gray-100">
              {row.map((cell, ci) => (
                <td key={ci} className="px-3 py-2 text-gray-700 text-xs">
